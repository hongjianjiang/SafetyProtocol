...
function construct25By241(msgNo1,msgNo2:indexType):indexType;
  var index : indexType;
      ---msg : Message;
  begin
   index := 0;
...
function construct26By1(msgNo1:indexType):indexType;
  var index : indexType;
      ---msg : Message;
  begin
   index := 0;
...
---pat24: m1 
procedure lookAddPat24(m1:Message; Var msg:Message; Var num : indexType);
  Var index : indexType;
  begin
    get_msgNo(m1,index); 
    num:=index;
...
---spat24: m1 
procedure constructSpat24(m1:Message; Var num: indexType);
  Var i, index : indexType;
  begin
   index:=0;
   i := 1;
...
---pat25: m1.bn 
procedure lookAddPat25(m1:Message; bn:NonceType; Var msg:Message; Var num : indexType);
  Var msg1,msg2: Message;
     index,i1,i2:indexType;
  begin
   index:=0;
   lookAddPat24(m1, msg1, i1);
   lookAddPat1(bn, msg2, i2);
   for i : indexType do
     if (msgs[i].msgType = concat & msgs[i].length=2) then
       if (msgs[i].concatPart[1]=i1 & msgs[i].concatPart[2]=i2) then
...
procedure isPat25(msg:Message; Var flag:boolean);
  ...
  begin
     ...
     flagPart2 := false;
     if(msg.msgType = concat) then
        isPat24(msgs[msg.concatPart[1]],flagPart1);
        isPat1(msgs[msg.concatPart[2]],flagPart2);
       if (flagPart1 & flagPart2) then 
         flag1 := true;
       endif;
...
---spat25: m1.bn 
procedure constructSpat25(m1:Message; bn:NonceType; Var num: indexType);
  Var i,index, i1, i2:indexType;
  begin
    index:=0;
    constructSpat24(m1, i1);
    constructSpat1(bn, i2);
    i := 1;
    while(i<= msg_end) do
      if (msgs[i].msgType = concat & msgs[i].length = 2) then
...
---pat26: ue 
procedure lookAddPat26(ue:NonceType; Var msg:Message; Var num : indexType);
  Var msg1: Message;
     index,i1:indexType;
  begin
   index:=0;
...
---spat26: ue 
procedure constructSpat26(ue:NonceType; Var num: indexType);
...
procedure cons24(m1:Message; Var msg:Message; Var num:indexType);
  begin
    clear msg;
    clear num;    lookAddPat24(m1,msg,num);
  end;
procedure destruct24(agmsg:Message; msg:Message; Var m1:Message);
  var msgNo:indexType;
  begin
    get_msgNo(msg,msgNo);
    m1:=msg;
  end;
procedure cons25(m1:Message; bn:NonceType; Var msg:Message; Var num:indexType);
  begin
    clear msg;
    clear num;    lookAddPat25(m1, bn,msg,num);
  end;
procedure destruct25(agmsg:Message; msg:Message; Var m1:Message; Var bn:NonceType);
  Var msgNum1,msgNum2: Message;
      k: KeyType;
  begin
    msgNum1 := msgs[msg.concatPart[1]];
...
procedure cons26(ue:NonceType; Var msg:Message; Var num:indexType);
  begin
    clear msg;
    clear num;    lookAddPat26(ue,msg,num);
  end;
procedure destruct26(agmsg:Message; msg:Message; Var ue:NonceType);
  Var msgNum1: Message;
      k: KeyType;
  begin
    msgNum1 := msgs[msg.concatPart[1]];
...
begin
   ...
   C_known[msgNo] := true;
   if(flag_pat25) then
     destruct25(agmsg,msg,roleC[i].locm1,roleC[i].locbn);
     if(matchTmp(roleC[i].locm1, roleC[i].m1) & matchNonce(roleC[i].locbn, roleC[i].bn))then
       ch[2].empty:=true;
       clear ch[2].msg;
       roleC[i].st := C2;
     endif;
...
begin
   ...
   C_known[msgNo] := true;
   if(flag_pat26) then
     destruct26(agmsg,msg,roleC[i].locue);
...
begin
   ...
   D_known[msgNo] := true;
   if(flag_pat25) then
     destruct25(agmsg,msg,roleD[i].locm1,roleD[i].locbn);
...
ruleset i: msgLen do
  ruleset j: roleBNums do
    rule "intruderEmitMsgIntoCh[1]"
      ch[1].empty=true & i <= pat4Set.length & pat4Set.content[i] != 0 & Spy_known[pat4Set.content[i]] ---& matchPat(msgs[pat4Set.content[i]], sPat4Set)
      ==>
      begin
...
ruleset i: msgLen do
  ruleset j: roleBNums do
    rule "intruderEmitMsgIntoCh[7]"
      ch[7].empty=true & i <= pat1Set.length & pat1Set.content[i] != 0 & Spy_known[pat1Set.content[i]] ---& matchPat(msgs[pat1Set.content[i]], sPat1Set)
      ==>
      begin
...
ruleset i: msgLen do
  ruleset j: roleBNums do
    rule "intruderEmitMsgIntoCh[11]"
      ch[11].empty=true & i <= pat10Set.length & pat10Set.content[i] != 0 & Spy_known[pat10Set.content[i]] ---& matchPat(msgs[pat10Set.content[i]], sPat10Set)
      ==>
      begin
...
ruleset i: msgLen do
  ruleset j: roleBNums do
    rule "intruderEmitMsgIntoCh[15]"
      ch[15].empty=true & i <= pat1Set.length & pat1Set.content[i] != 0 & Spy_known[pat1Set.content[i]] ---& matchPat(msgs[pat1Set.content[i]], sPat1Set)
      ==>
      begin
        if (!emit[pat1Set.content[i]]) then  
          clear ch[15];
...
ruleset i: msgLen do
  ruleset j: roleCNums do
    rule "intruderEmitMsgIntoCh[2]"
      ch[2].empty=true & i <= pat13Set.length & pat13Set.content[i] != 0 & Spy_known[pat13Set.content[i]] ---& matchPat(msgs[pat13Set.content[i]], sPat13Set)
      ==>
      begin
...
ruleset i: msgLen do
  ruleset j: roleCNums do
    rule "intruderEmitMsgIntoCh[8]"
      ch[8].empty=true & i <= pat15Set.length & pat15Set.content[i] != 0 & Spy_known[pat15Set.content[i]] ---& matchPat(msgs[pat15Set.content[i]], sPat15Set)
      ==>
      begin
...
ruleset i: msgLen do
  ruleset j: roleANums do
    rule "intruderEmitMsgIntoCh[10]"
      ch[10].empty=true & i <= pat18Set.length & pat18Set.content[i] != 0 & Spy_known[pat18Set.content[i]] ---& matchPat(msgs[pat18Set.content[i]], sPat18Set)
      ==>
      begin
...
ruleset i: msgLen do
  ruleset j: roleCNums do
    rule "intruderEmitMsgIntoCh[12]"
      ch[12].empty=true & i <= pat23Set.length & pat23Set.content[i] != 0 & Spy_known[pat23Set.content[i]] ---& matchPat(msgs[pat23Set.content[i]], sPat23Set)
      ==>
      begin
...
ruleset i: msgLen do
  ruleset j: roleANums do
    rule "intruderEmitMsgIntoCh[14]"
      ch[14].empty=true & i <= pat11Set.length & pat11Set.content[i] != 0 & Spy_known[pat11Set.content[i]] ---& matchPat(msgs[pat11Set.content[i]], sPat11Set)
      ==>
      begin
...
ruleset i: msgLen do
  ruleset j: roleCNums do
    rule "intruderEmitMsgIntoCh[16]"
      ch[16].empty=true & i <= pat1Set.length & pat1Set.content[i] != 0 & Spy_known[pat1Set.content[i]] ---& matchPat(msgs[pat1Set.content[i]], sPat1Set)
      ==>
      begin
        if (!emit[pat1Set.content[i]]) then  
          clear ch[16];
...
ruleset i: msgLen do
  ruleset j: roleANums do
    rule "intruderEmitMsgIntoCh[18]"
      ch[18].empty=true & i <= pat1Set.length & pat1Set.content[i] != 0 & Spy_known[pat1Set.content[i]] ---& matchPat(msgs[pat1Set.content[i]], sPat1Set)
      ==>
      begin
        if (!emit[pat1Set.content[i]]) then  
          clear ch[18];
...
ruleset i: msgLen do
  ruleset j: roleDNums do
    rule "intruderEmitMsgIntoCh[3]"
      ch[3].empty=true & i <= pat13Set.length & pat13Set.content[i] != 0 & Spy_known[pat13Set.content[i]] ---& matchPat(msgs[pat13Set.content[i]], sPat13Set)
      ==>
      begin
        if (!emit[pat13Set.content[i]]) then  
          clear ch[3];
...
ruleset i: msgLen do
  ruleset j: roleBNums do
    rule "intruderEmitMsgIntoCh[5]"
      ch[5].empty=true & i <= pat1Set.length & pat1Set.content[i] != 0 & Spy_known[pat1Set.content[i]] ---& matchPat(msgs[pat1Set.content[i]], sPat1Set)
      ==>
      begin
        if (!emit[pat1Set.content[i]]) then  
          clear ch[5];
...
ruleset i: msgLen do
  ruleset j: roleBNums do
    rule "intruderEmitMsgIntoCh[9]"
      ch[9].empty=true & i <= pat2Set.length & pat2Set.content[i] != 0 & Spy_known[pat2Set.content[i]] ---& matchPat(msgs[pat2Set.content[i]], sPat2Set)
      ==>
      begin
...
ruleset i: msgLen do
  ruleset j: roleBNums do
    rule "intruderEmitMsgIntoCh[13]"
      ch[13].empty=true & i <= pat9Set.length & pat9Set.content[i] != 0 & Spy_known[pat9Set.content[i]] ---& matchPat(msgs[pat9Set.content[i]], sPat9Set)
      ==>
      begin
...
ruleset i: msgLen do
  ruleset j: roleCNums do
    rule "intruderEmitMsgIntoCh[17]"
      ch[17].empty=true & i <= pat1Set.length & pat1Set.content[i] != 0 & Spy_known[pat1Set.content[i]] ---& matchPat(msgs[pat1Set.content[i]], sPat1Set)
      ==>
      begin
        if (!emit[pat1Set.content[i]]) then  
          clear ch[17];
...
ruleset i: msgLen do
  ruleset j: roleCNums do
    rule "intruderEmitMsgIntoCh[4]"
      ch[4].empty=true & i <= pat1Set.length & pat1Set.content[i] != 0 & Spy_known[pat1Set.content[i]] ---& matchPat(msgs[pat1Set.content[i]], sPat1Set)
      ==>
      begin
        if (!emit[pat1Set.content[i]]) then  
          clear ch[4];
  ...
  rule "deconcat 25" --pat25
    ...
    begin
      ...
      if (!Spy_known[msgs[pat25Set.content[i]].concatPart[1]]) then
        ...
        isPat24(msgs[msgPat1],flagPat1);
        if (flagPat1) then
          if(!exist(pat24Set,msgPat1)) then
             pat24Set.length:=pat24Set.length+1;
             pat24Set.content[pat24Set.length] := msgPat1;
          endif;
        endif;
      endif;
...
ruleset i1: msgLen do
  ruleset i2: msgLen do 
    rule "enconcat 25"	---pat25
      i1<=pat24Set.length & Spy_known[pat24Set.content[i1]] &
      i2<=pat1Set.length & Spy_known[pat1Set.content[i2]] &
      matchPat(msgs[construct25By241(pat24Set.content[i1],pat1Set.content[i2])], sPat25Set)&
      !Spy_known[construct25By241(pat24Set.content[i1],pat1Set.content[i2])] 
      ==>
      var concatMsgNo:indexType;
      begin
        put "rule enconcat25\n";
        concatMsgNo := construct25By241(pat24Set.content[i1],pat1Set.content[i2]);
        Spy_known[concatMsgNo]:=true;
...
ruleset i:msgLen do 
  rule "deconcat 26" --pat26
    ...
    !(Spy_known[msgs[pat26Set.content[i]].concatPart[1]])
    ==>
...
ruleset i1: msgLen do 
    rule "enconcat 26"	---pat26
      i1<=pat1Set.length & Spy_known[pat1Set.content[i1]] &
      matchPat(msgs[construct26By1(pat1Set.content[i1])], sPat26Set)&
      !Spy_known[construct26By1(pat1Set.content[i1])] 
      ==>
      var concatMsgNo:indexType;
      begin
        put "rule enconcat26\n";
        concatMsgNo := construct26By1(pat1Set.content[i1]);
        Spy_known[concatMsgNo]:=true;
