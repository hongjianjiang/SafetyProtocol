...
ruleset i: msgLen do
  ruleset j: roleANums do
    rule "intruderEmitMsgIntoCh[6]"
      IntruEmit5 = true & roleA[j].st = A2 & ch[6].empty=true & i <= pat15Set.length & pat15Set.content[i] != 0 & Spy_known[pat15Set.content[i]] & !emit[pat15Set.content[i]] ---& matchPat(msgs[pat15Set.content[i]], sPat15Set)
      ==>
      begin
         clear ch[6];
...
ruleset i: msgLen do
  ruleset j: roleANums do
    rule "intruderEmitMsgIntoCh[10]"
      IntruEmit9 = true & roleA[j].st = A4 & ch[10].empty=true & i <= pat19Set.length & pat19Set.content[i] != 0 & Spy_known[pat19Set.content[i]] & !emit[pat19Set.content[i]] ---& matchPat(msgs[pat19Set.content[i]], sPat19Set)
      ==>
      begin
         clear ch[10];
...
ruleset i: msgLen do
  ruleset j: roleANums do
    rule "intruderEmitMsgIntoCh[14]"
      IntruEmit13 = true & roleA[j].st = A6 & ch[14].empty=true & i <= pat12Set.length & pat12Set.content[i] != 0 & Spy_known[pat12Set.content[i]] & !emit[pat12Set.content[i]] ---& matchPat(msgs[pat12Set.content[i]], sPat12Set)
      ==>
      begin
         clear ch[14];
...
ruleset i: msgLen do
  ruleset j: roleBNums do
    rule "intruderEmitMsgIntoCh[11]"
      IntruEmit10 = true & roleB[j].st = B9 & ch[11].empty=true & i <= pat11Set.length & pat11Set.content[i] != 0 & Spy_known[pat11Set.content[i]] & !emit[pat11Set.content[i]] ---& matchPat(msgs[pat11Set.content[i]], sPat11Set)
      ==>
      begin
         clear ch[11];
...
ruleset i: msgLen do
  ruleset j: roleBNums do
    rule "intruderEmitMsgIntoCh[13]"
      IntruEmit12 = true & roleB[j].st = B11 & ch[13].empty=true & i <= pat10Set.length & pat10Set.content[i] != 0 & Spy_known[pat10Set.content[i]] & !emit[pat10Set.content[i]] ---& matchPat(msgs[pat10Set.content[i]], sPat10Set)
      ==>
      begin
         clear ch[13];
...
ruleset i: msgLen do
  ruleset j: roleCNums do
    rule "intruderEmitMsgIntoCh[2]"
      IntruEmit1 = true & roleC[j].st = C1 & ch[2].empty=true & i <= pat14Set.length & pat14Set.content[i] != 0 & Spy_known[pat14Set.content[i]] & !emit[pat14Set.content[i]] ---& matchPat(msgs[pat14Set.content[i]], sPat14Set)
      ==>
      begin
         clear ch[2];
...
ruleset i: msgLen do
  ruleset j: roleCNums do
    rule "intruderEmitMsgIntoCh[8]"
      IntruEmit7 = true & roleC[j].st = C5 & ch[8].empty=true & i <= pat16Set.length & pat16Set.content[i] != 0 & Spy_known[pat16Set.content[i]] & !emit[pat16Set.content[i]] ---& matchPat(msgs[pat16Set.content[i]], sPat16Set)
      ==>
      begin
         clear ch[8];
...
ruleset i: msgLen do
  ruleset j: roleCNums do
    rule "intruderEmitMsgIntoCh[12]"
      IntruEmit11 = true & roleC[j].st = C7 & ch[12].empty=true & i <= pat24Set.length & pat24Set.content[i] != 0 & Spy_known[pat24Set.content[i]] & !emit[pat24Set.content[i]] ---& matchPat(msgs[pat24Set.content[i]], sPat24Set)
      ==>
      begin
         clear ch[12];
...
ruleset i: msgLen do
  ruleset j: roleDNums do
    rule "intruderEmitMsgIntoCh[3]"
      IntruEmit2 = true & roleD[j].st = D1 & ch[3].empty=true & i <= pat14Set.length & pat14Set.content[i] != 0 & Spy_known[pat14Set.content[i]] & !emit[pat14Set.content[i]] ---& matchPat(msgs[pat14Set.content[i]], sPat14Set)
      ==>
      begin
         clear ch[3];
